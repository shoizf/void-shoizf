...
# Installer script for 'awww' (wallpaper daemon)
...
if [ -d "$AWWW_DIR" ]; then
  ...
  cd "$REPO_ROOT" # Changed from $OLDPWD to repo root for consistency
else
...
if ! check_rust_version; then
  log_info "Installing Rust from source via rustup..."
